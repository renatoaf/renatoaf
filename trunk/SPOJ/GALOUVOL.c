	...
	for (k = 0; k < 6; k++) {
		...
		if (podeVisitar(x, y, lin, col)) {
			if (estados[i][j] == estados[x][y]) {
				// Uma engrenagem estÃ¡ bloqueada quando ela
				// estÃ¡ tentando girar em ambos sentidos ao mesmo tempo.
	...
	while (1) {
		...
		for (i = 0; i < linhas; i++) {
			...
			for (j = 0; j < colunas; j++) {
				motor[i][j] = linha[j];
				if (motor[i][j] == MOVE) {
					// SÃ£o inicialmente ativadas e tentam giram em sentido
					// horÃ¡rio
					estados[i][j] = HORARIO;
				} else if (motor[i][j] == N_MOVE) {
					// Uma engrenagem estÃ¡ livre quando ela nÃ£o Ã© ativada
					estados[i][j] = LIVRE;
				} else {
					estados[i][j] = N_TEM;
				}
